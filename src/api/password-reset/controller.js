...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Siwes Api Yabatech account.<br>
        ...
        &mdash; Siwes Api Yabatech Team
      `
      return sendMail({ toEmail: email, subject: 'Siwes Api Yabatech - Password Reset', content })
    })
